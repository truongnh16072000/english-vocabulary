...
import { 
  ...
  Sparkles, Layers, BookMarked, Sidebar as SidebarIcon, Trophy, Heart, Briefcase
} from 'lucide-react';
...
const Sidebar = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <motion.aside
      ...
      className="sidebar-glass h-screen sticky top-0 left-0 z-40 flex flex-col shadow-2xl overflow-hidden transition-[width] duration-300 hidden lg:flex"
    >
      {/* Header */}
      <div className={`p-6 flex items-center shrink-0 ${isCollapsed ? 'justify-center px-0' : 'justify-between'}`}>
        {!isCollapsed && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
