...
const A2Vocabulary = () => {
  ...
  const [quizSize, setQuizSize] = useState(() => {
    const saved = localStorage.getItem('a2_quiz_size');
    return saved ? parseInt(saved, 10) : 10;
  });
  const [quizActive, setQuizActive] = useState(() => {
    return localStorage.getItem('a2_quiz_active') === 'true';
  });
  const [quizPool, setQuizPool] = useState(() => {
    const saved = localStorage.getItem('a2_quiz_pool');
    return saved ? JSON.parse(saved) : [];
  });
  const [quizIndex, setQuizIndex] = useState(() => {
    const saved = localStorage.getItem('a2_quiz_index');
    return saved ? parseInt(saved, 10) : 0;
  });
  const [quizScore, setQuizScore] = useState(() => {
    const saved = localStorage.getItem('a2_quiz_score');
    return saved ? parseInt(saved, 10) : 0;
  });
  const [quizFinished, setQuizFinished] = useState(() => {
    return localStorage.getItem('a2_quiz_finished') === 'true';
  });
  const [selectedAnswer, setSelectedAnswer] = useState(null);
  const [quizOptions, setQuizOptions] = useState([]);

  useEffect(() => {
    localStorage.setItem('a2_quiz_size', quizSize);
    localStorage.setItem('a2_quiz_active', quizActive);
    localStorage.setItem('a2_quiz_pool', JSON.stringify(quizPool));
    localStorage.setItem('a2_quiz_index', quizIndex);
    localStorage.setItem('a2_quiz_score', quizScore);
    localStorage.setItem('a2_quiz_finished', quizFinished);
    ...
    <div className="bg-white text-slate-900 font-sans pb-10">
      ...
      <div className="max-w-6xl mx-auto px-4 py-4 border-b border-slate-100 mb-6">
        <nav className="flex bg-slate-100 p-1.5 rounded-2xl w-fit">
          <button onClick={() => { setActiveTab('list'); playSound('select'); }} className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'list' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-indigo-500'}`}>
            <LayoutGrid className="w-4 h-4" /> Danh sách ({filteredVocab.length})
          </button>
          <button onClick={() => { setActiveTab('flashcard'); playSound('select'); }} className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'flashcard' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-indigo-500'}`}>
            <Brain className="w-4 h-4" /> Thẻ nhớ
          </button>
          <button onClick={() => { setActiveTab('quiz'); setQuizIndex(0); setQuizFinished(false); setQuizScore(0); playSound('select'); }} className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'quiz' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-indigo-500'}`}>
            ...
            {topics.map(topic => (
              <button 
                ...
                onClick={() => {
                  ...
                  setQuizActive(false); // Reset quiz when filter changes to avoid logic confusion
                  setQuizIndex(0); 
                  setQuizFinished(false);
                  playSound('select');
                }}
        ...
        {activeTab === 'list' && (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredVocab.map((item, idx) => (
              <div key={idx} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all group animate-fade-in">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <h3 className="text-xl font-black text-slate-800 capitalize group-hover:text-indigo-600 transition-colors">{item.word}</h3>
                    <p className="text-slate-400 text-sm font-medium mt-1">{item.ipa} • <span className="text-indigo-400 italic">{item.pos}</span></p>
                  </div>
                  <div className="flex flex-col gap-2">
                    <button onClick={() => speak(item.word)} className="p-3 rounded-full bg-slate-50 text-indigo-500 hover:bg-indigo-600 hover:text-white transition shadow-sm active:scale-90">
                ...
                <p className="text-slate-800 font-bold text-base mb-4">💡 {item.meaning}</p>
                <div className="bg-slate-50 p-4 rounded-2xl border-l-4 border-indigo-400 mb-4">
                  <p className="text-sm text-slate-600 mb-2 leading-relaxed italic">"{item.example}"</p>
                  <p className="text-[11px] text-slate-400 font-bold uppercase">{item.translation}</p>
                </div>
                
                <button 
                  onClick={() => handleOpenAiModal(item)}
                  className="w-full py-3 rounded-xl bg-indigo-50 text-indigo-600 font-bold text-xs flex items-center justify-center gap-2 hover:bg-indigo-600 hover:text-white transition-all group-hover:shadow-lg group-hover:shadow-indigo-100"
                >
                  <Sparkles className="w-4 h-4" /> TRỢ GIẢNG AI
                </button>
              </div>
            ))}
            {filteredVocab.length === 0 && (
              <div className="col-span-full py-32 text-center">
                ...
                <p className="text-slate-400 font-bold italic text-lg">Không tìm thấy từ nào phù hợp...</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="flex flex-col items-center py-12 space-y-10">
            ...
            <div className="flex gap-8 items-center">
              <button 
                ...
                className="p-5 rounded-full bg-white border border-slate-200 text-slate-400 hover:text-indigo-600 hover:border-indigo-500 disabled:opacity-30 transition-all shadow-xl active:scale-95"
              >
                <ChevronLeft className="w-8 h-8" />
              </button>
              <button 
                onClick={() => speak(filteredVocab[currentFlashcard].word)} 
                className="px-12 py-5 rounded-full bg-indigo-600 text-white font-black hover:bg-indigo-700 shadow-2xl flex items-center gap-4 transition-all hover:scale-105 active:scale-95 ring-8 ring-indigo-50"
              >
                <Volume2 className="w-7 h-7" /> PHÁT ÂM
              </button>
              <button 
                ...
                <div className="grid grid-cols-2 gap-4 mb-10">
                  {[10, 20, 50, 'all'].map(size => (
                    <button 
                      ...
                      className="p-5 rounded-3xl border-2 border-slate-100 hover:border-indigo-400 hover:bg-indigo-50 transition-all font-black text-slate-700 active:scale-95 group"
                    >
                      <div className="text-2xl group-hover:text-indigo-600">{size === 'all' ? filteredVocab.length : size}</div>
                      <div className="text-[10px] uppercase tracking-widest text-slate-400 group-hover:text-indigo-400">{size === 'all' ? 'Tất cả từ' : 'Từ vựng'}</div>
                    </button>
                  ))}
                </div>
              ...
              <div className="bg-white p-10 rounded-[40px] shadow-2xl border border-slate-100 animate-slide-up">
                ...
                <div className="text-center mb-12">
                  ...
                  <button 
                    ...
                    className="p-4 bg-indigo-50 rounded-full text-indigo-500 hover:text-indigo-700 transition-all hover:scale-110 active:scale-90"
                  >
              ...
              <div className="bg-white p-16 rounded-[50px] shadow-2xl text-center border border-indigo-50 animate-zoom-in">
                ...
                <div className="grid grid-cols-2 gap-4 mb-12">
                   <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                     <div className="text-3xl font-black text-indigo-600">{Math.round((quizScore / quizPool.length) * 100)}%</div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-center justify-center">
                  <button 
                    ...
                    className="w-full sm:w-auto px-12 py-5 bg-indigo-600 text-white rounded-3xl font-black text-lg shadow-xl hover:bg-indigo-700 transition-all active:scale-95"
                  >
                    LÀM BÀI MỚI
                  </button>
                </div>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in" style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh' }}>
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto flex-grow bg-slate-50/50">
              ...
              {/* AI Response Area */}
              <div className="bg-white p-6 rounded-2xl border border-indigo-100 shadow-sm min-h-[200px]">
                {isAiLoading ? (
      ...
      <footer className="max-w-6xl mx-auto px-6 py-10 text-center text-slate-300">
        <div className="flex items-center justify-center gap-2 font-black tracking-widest uppercase text-xs">
          ...
          A2 Level Vocabulary • Có AI Gemini Hỗ Trợ
        </div>
      </footer>
    </div>
  );
